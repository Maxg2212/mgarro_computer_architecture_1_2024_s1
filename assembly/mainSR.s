...
.section .data
  ...
  constante_div: .word 0x00028000 @ Valor de 1/(1 - alpha)
  ...
  name_input: .asciz "output.bin"
  name_output: .asciz "outputsr.bin"  
...
_reverbsr:
  ...
  _end_if_else:
    @ Se almacena en r4 el valor de la lista x(n-k)
  ...
  _mult_end2:
  @r8 = 1/1-alpha * (x(n) - alpha * x(n-k))
  

  add r3, r3, #1  @ Incrementar el valor de r3 en uno

  str r8, [r0] @ se almacena la posicion y(n) en la lista
  ...
  blt _reverbsr         @ Repetir el ciclo si el valor en r3 es menor que el valor dado
  beq _loadfiles
...
_mult_pfijo2:
  ...
  add r8, r4, r7   @ high<<<16 + low>>>16 + mid (aun no se está utilizando buffer output)
  b _mult_end2

_equals:
    ldr r5, =buffer_input @ Cargar la dirección de constante_k en r5
    ldr r4, [r5]
